...
function getTemplate({ items }: { items: string[] }): Node {
...
class WorkList extends HTMLElement {
  connectedCallback() {
    ...
    const formattedItems = (items as Items).map(
      ({ count, title, tags, year, url, code }) => {
  ...
  get src(): WorkListSrc {
    return this.getAttribute("src") as WorkListSrc;
  }
}
